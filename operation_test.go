...
func TestPerformOperationEf(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		n        uint
		sets     []intIterator
		expected intIterator
	}{
		"LE N=1 one set": {
			opFunc:   OpFuncLE,
			n:        1,
			sets:     []intIterator{newSlice([]int{1, 2, 3})},
			expected: newSlice([]int{}),
		},
		"LE N=1 two sets": {
			opFunc: OpFuncLE,
			n:      1,
		...
		"LE N=2 three sets": {
			...
			expected: newSlice([]int{-1, 0, 2, 3, 4, 7, 8, 9, 10}),
		},
